...
export function useAccountPageAnimation() {
  useGSAP(() => {
...
/* Crossfades whichever panel just became active. Panels stay mounted (so a
   half-typed form survives a tab switch) — only the incoming one animates. */
export function useTabCrossfade(activeTab) {
  useGSAP(() => {
    const panel = document.querySelector(".account-panel.is-active");
    if (!panel) return;
